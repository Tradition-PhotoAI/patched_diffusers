...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def modified_spacer(self, start, end, num_points):
        # Calculate the factor for exponential growth
        exp_factor = np.log(end / start)
        
        # Generate points with a non-linear distribution in the exponent
        # Using a power of 2 to make the growth accelerate more towards the end
        points = start * np.exp(np.linspace(0, exp_factor, num_points)**2 / exp_factor)
        return points
